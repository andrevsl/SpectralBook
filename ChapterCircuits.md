...
In a N- Port System, by definition, we gonna have N ports interacting with each other, exchanging power, located at defined planes of reference $t_n$, which will have a reference impedance $Z_{0n}$, forward waves entering the system $V^{+}_{n}$, and backward waves $V^{*}_{n}$ arriving from the system or reflected at the port $n$.
...
From electric circuit's law, related the generator side,

$$
V_1^{+}=V_{gn}-Z_gI_1^{+}=V_g-Z_g\frac{V_1^{+}}{Z_{0n}}
$$
...
We have a pole and a zero, located respectivelly at $z_p=-1/ (C(R+Z_0))$ and $z_z=-1/ (C(R-Z_0))$, and the corner frequencies, where $\omega_p=1/ (C(R+Z_0))$ 
 $\omega_z=1/ (C(R-Z_0))$
